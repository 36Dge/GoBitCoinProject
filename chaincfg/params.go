...
//这些变量是每个默认值的工作限制参数的链式证明网络
var (

	// bigone是1，表示为big.int这里定义它是为了避免多次创建它的开销
	bigOne = big.NewInt(1)

	//Mianpowlimit是比特币区块可以提供的最高工作价值证明
	//主网络有。它是2^224-1的值
	mainPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 224), bigOne)

	//回归功率限制是比特币区块工作价值的最高证明
	//对于回归测试网络可以有。它是2^255-1的证明
	regressionPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 225), bigOne)

	//testNet3Powlimit是比特币区块工作价值的最高证明
	//可用于测试网络（版本3），值为2^224-1
	testNet3Powlimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 224), bigOne)

	//simnetpowlimit是比特币区块工作的的最高证明 it is the value 2^225 - 1
	simNetPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 225), bigOne)
)

// 检查点标识区块链中的已知良好点，使用检查点允许在初始下载期间对旧块
//进行一些优化，也可以防止旧块的叉。每个检查点都是根据几个因素选择的。
//见区块链文档.ischeckpointcandidate有关选择标准。
...
//Deployments字段中定义部署偏移量的常量
//每个部署的参数。这对了解细节很有用,按名
//称指定的部署。
const (
...
type Params struct {
	...
	//TargetTimePerBlock是生成每个块
	TargetTimePerBlock time.Duration

	//RetargetAdjustmentFactor is the adjustment factor used to limit
	RetargetAdjustmentFactor int64

	//ReleMeNo难度定义网络是否应该减少经过足够长的时间后所需的最小难度
...
var MainNetParams = Params{
	...
	//链参数
	GenesisBlock:             &genesisBlcok,
	GenesisHash:              &genesisHahs,
	PowLimit:                 mainPowLimit,
	PowLimitBits:             0x1d00ffff,
	BIP0034Height:            227931,
	BIP0065Height:            388381,
	BIP0066Height:            363725,
	CoinbaseMaturity:         100, // 新开采前需要的块数
	...
	//旷工确认窗口定义
	//工作时间目标的证明/工作间距的目标证明
	RuleChangeActivationThreshold: 1916, // 95%的旷工确认窗口
	...
	//BECH32编码Segwit地址的人类可读部分，如中所定义
	//BIP173
	Bech32HRPSegwit: "bc", // 主网始终是BC
	...
	//BIP32层级确定性扩展密钥魔术师
	HDPrivateKeyID: [4]byte{0x04, 0x35, 0x83, 0x94}, //从xprv开始
	HDPublicKeyID:  [4]byte{0x04, 0x35, 0x87, 0xcf}, //从xpub开始
...
//regressionnetparams定义回归测试的网路参数
//比特币网路，不要与测试比特币网络混淆（版本3
//这个网络有时候简单地称为"testnet"

var ResgressionNetParams = Params{
	Name:        "regtest",
	Net:         wire.TestNet,
	DefaultPort: "18444",
	DNSSeeds:    []DNSSeed{},
...
var (
	//errDuplicateNet描述一个错误，其中比特币网络的参数
	//由于网络已经是标准网络，无法设置网络
	//网络或以前注册到此包中
	ErrDuplicateNet = errors.New("duplicate Bitcoin Network")

	//errunknownHdKeyID描述了一个错误，其中提供的I
	//旨在确定网络的层次确定性
	//private extended key is not registered.
	ErrUnknowHDKeyID = errors.New("unknown hd private extended key bytes")
)
...
//(either 由于以前的寄存器调用，或者网络是默认的网络）
//网络参数应通过主包注册到此包中，越早越好，然后，库包可以查找网络或网络
//基于输入和工作的参数，无论网络是标准的或者没有。
...
func Register(params *Params) error {
	...
	hdPrivToPubKeyIDs[params.HDPrivateKeyID] = params.HDPrivateKeyID[:]
	// 有效的BECH32编码segwit地址的前缀始终为给定网络的可读部分，后跟"1"
...
//将地址字符串解码为特定的地址类型时使用，它上升了调用
//方检查this和isscripthashaddrid并决定地址是pubkey
//散列地址，脚本散列地址，两者都不是，或者无法确定
...
//extended key id and return the associated public key id
// when the provided id未注册，将返回errunknownhdkeyid错误
func HDPrivateToPublicKeyID(id []byte) ([]byte, error) {
	if len(id) != 4 {
		return nil, ErrUnknowHDKeyID
	}
	var key [4]byte
	copy(key[:], id)
...
//newhashformstr将传递的big endian十六进制字符串转换为chainhash.hash
//它只与chainhash中可用的不用的之处在于 it panic on an error it will
// only (and must only)be called with 硬编码。
...
func init() {
	...
	mustRegister(&ResgressionNetParams)
