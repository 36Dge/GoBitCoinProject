...
//mediantime provides an implementation of the mediantimesoure interface .
//it is limited to maxmediantimeentries includes the same buggy behavior as
//the time offset mechanism in bitcoin core this is necessary because it it
//used in the concensus code.
...
//addtiemsample adds a time sample that is used when detering the median
//time of the added samples.
func (m *medianTime) AddTimeSample(sourceID string, timeVal time.Time) {
	m.mtx.Lock()
	defer m.mtx.Unlock()

	//dont add time data from the  same source
