...
//format locktime returns a trasaction lock time as a human-readable string
func formatLockTime(lockTime uint32) string {
	//the locktime field of a transaction is either a block height at
	//which time tracnsaction is finalized or a timestamp depending on if
	//the value is before the locktimethreshold. when it is under the
	//threshold it is a block heigth.
...
//invsummary returns an inventory message as a human-radble string.
...
//not all message have or need a summary .this is used for debug logging .
